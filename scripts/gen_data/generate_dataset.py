...
def main():
    ...
    parser.add_argument("--t_rx", nargs='?', type=float, default=150., help="t_rx parameter in noise generation")
    parser.add_argument("--three-channels", default=False, action="store_true", help="Output data with 3 channels only \
         (rather than 4). Useful when generating simulated data with one layer of mask that will be double masked.")
    ...
    if existing_vis is not None: # existing (real) visibilities
        ...
        vis_list, _ = generate_simulated_vis_wrapper(n_examples, num_jd, start_freq, end_freq, num_reduced_channels, False, t_rx)

    # crop visibilities (may not be necessary)
    vis_list = crop_vis(vis_list, 512, save=False)

    # create dataset
    create_dataset(vis_list, mask_list, save, existing_vis, three_channels)
